...
const schema = Yup.object({
  name: Yup.string().required("Tên danh mục không được để trống").min(3, "Tên danh mục phải có ít nhất 3 ký tự").max(50, "Tên danh mục không được vượt quá 50 ký tự"),
  slug: Yup.string()
    .required("Slug không được để trống").min(3, "Slug phải có ít nhất 3 ký tự").max(50, "Slug không được vượt quá 50 ký tự")
    .matches(
      /^[a-z0-9]+(?:-[a-z0-9]+)*$/,
...
const CreateCategory = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Reset form hoặc chuyển hướng trang sau khi tạo thành công
    } catch (error) {
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-md shadow-lg max-w-sm w-full">
            ...
            <p className="mb-4">Bạn có chắc chắn muốn tạo danh mục này?</p>
